<script lang="ts">
  ...
  let prep_time_minutes = $state<number | undefined>(recipe.prep_time_minutes ?? undefined);
  let cook_time_minutes = $state<number | undefined>(recipe.cook_time_minutes ?? undefined);
  let source_url = $state(recipe.source_url ?? "");
  let author = $state(recipe.author ?? "");
  let is_public = $state(recipe.is_public);
...
<div class="flex min-h-screen flex-col items-center py-12 px-4">
  <GlassCard accent class="w-full max-w-2xl">
    <div class="flex items-center justify-between">
      ...
      <Button variant="ghost" href="/recipes">Cancel</Button>
    </div>

    <!-- Basic info -->
    <div class="flex flex-col gap-2">
      <label class="text-sm font-semibold text-white/70">Name *</label>
      <input bind:value={name} class="w-full rounded-lg border {nameError ? 'border-red-400/70 focus:border-red-400' : 'border-white/30 focus:border-violet-400'} bg-white/10 px-3 py-2 text-sm text-white shadow-sm placeholder:text-white/30 focus:outline-none" />
    ...
    <div class="flex flex-col gap-2">
      ...
      <textarea bind:value={description} class="w-full rounded-lg border border-white/30 bg-white/10 px-3 py-2 text-sm text-white shadow-sm placeholder:text-white/30 focus:outline-none focus:border-violet-400 resize-none" rows="2"></textarea>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="flex flex-col gap-2">
        ...
        <input bind:value={author} class="w-full rounded-lg border border-white/30 bg-white/10 px-3 py-2 text-sm text-white shadow-sm placeholder:text-white/30 focus:outline-none focus:border-violet-400" />
      </div>
    </div>
    ...
    <Button variant="primary" onclick={handleSubmit} class="w-full mt-2">Save Changes</Button>
  </GlassCard>
</div>
